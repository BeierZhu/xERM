...
from loss.xERMLoss import xERMLoss
...
class model ():
    def __init__(self, config, data, test=False, test_imb_ratio=1.0, test_reverse=False):
        ...
        # Setup logger
        self.logger = Logger(self.training_opt['exp_dir'])
        ...
        if not test:
            self.tensorboard = SummaryWriter(log_dir=f"{self.training_opt['exp_dir']}/tensorboard")
        self.current_step = 0
        ...
        if 'exp_dir' in self.config and self.config['exp_dir'] is not None:
            self.load_model(self.config['exp_dir'])
        ...
        if not self.test_mode:
            ...
            # Set up log file
            self.log_file = os.path.join(self.training_opt['exp_dir'], 'log.txt')
            self.logger.log_cfg(self.config)
        else:
            if test_reverse:
                self.log_file = os.path.join(self.training_opt['exp_dir'], f'test-reverse_imb{test_imb_ratio}.txt')
            else:
                self.log_file = os.path.join(self.training_opt['exp_dir'], f'test_imb{test_imb_ratio}.txt')
            self.logger.log_cfg(self.config)
    ...
    def xERM_train(self):
        ...
        self.distil_log = os.path.join(self.training_opt['exp_dir'], f'{time_str}_distil_log.txt')
        networks_defs = self.config['networks']
        xERM_networks = {}
        ...
        for key, val in networks_defs.items():
            ...
            # Optimizer list
            optim_params = val['xERM_optim_params']
            ...
            xERM_optim_params_dict[key] = {'params': xERM_networks[key].parameters(),
                                                ...
                                                'momentum': optim_params['momentum'],
                                                'weight_decay': optim_params['weight_decay']}
            exp_dir = os.path.join(self.training_opt['exp_dir'], 'final_model_checkpoint.pth')
            print('Loading xERM model from %s' % (exp_dir))
            checkpoint = torch.load(exp_dir)          
        ...
        gamma = self.config['training_opt']['gamma']
        print(f'=> gamma {gamma}')
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            for step, (inputs, labels, indexes) in enumerate(self.data['train']):
                ...
                logits_imba, _ = self.networks['classifier'](features, None, self.embed_mean)
                logits_ba = logits_imba - torch.log(self.prior_distribution["train"]).cuda() + torch.log(self.prior_distribution["test"]).cuda()
                # forward xERM model
                xERM_features = xERM_networks['feat_model'](inputs)
                logits_xERM, _ = xERM_networks['classifier'](xERM_features, None, self.embed_mean)
                # calculate loss
                loss = xERM_loss(logits_imba, logits_ba, logits_xERM, labels)
                losses.update(loss.item())
                _, te_preds = torch.max(logits_imba, 1)
                _, tde_preds = torch.max(logits_ba, 1)
                _, xERM_preds = torch.max(logits_xERM, 1)
                imba_accs.update((te_preds==labels).sum().item()/labels.size(0), n=labels.size(0))
                ba_accs.update((tde_preds==labels).sum().item()/labels.size(0), n=labels.size(0))
                ...
                if step % self.training_opt['display_step'] == 0:
                    print_w(f'Epoch: [{epoch}][{step}/{step_length+1}] '
                          ...
                          f'Imbalanced Acc {imba_accs.avg:.3f} '
                          f'Balanced Acc {ba_accs.avg:.3f} '
                          f'xERM Acc {xERM_accs.avg:.3f}', self.distil_log)
            ...
            total_ba_preds = torch.empty(0, dtype=torch.long).cuda()
            total_imba_preds = torch.empty(0, dtype=torch.long).cuda()

            for key, model in xERM_networks.items():
                model.eval()
            torch.cuda.empty_cache()
            with torch.no_grad():
                for step, (inputs, labels, indexes) in tqdm(enumerate(self.data['val'])):
                    inputs, labels = inputs.cuda(), labels.cuda()
                    # forward  model
                    features = self.networks['feat_model'](inputs)
                    logits_imba, _ = self.networks['classifier'](features, None, self.embed_mean)
                    ...
                    total_ba_preds = torch.cat((total_ba_preds, tde_preds))
                    total_imba_preds = torch.cat((total_imba_preds, te_preds))
            ...
            self.low_acc_top1, \
            self.cls_accs = shot_acc(total_preds,
                                     total_labels, 
                                     self.data['train'],
                                     acc_per_cls=True)
                                     
            # Top-1 accuracy and additional string
            if xERM_accs.avg > best_acc:
                best_acc = xERM_accs.avg
                print_w('=> Find better xERM model', self.distil_log)
                model_weights = {}
                ...
                exp_dir = os.path.join(self.training_opt['exp_dir'],
                                 'best_xERM_model_checkpoint.pth')
                torch.save(model_states, exp_dir)


            print_w('xERM ---------', self.distil_log)
            print_w(f'Many_shot_accuracy_top1: {self.many_acc_top1:.3f} ' 
                  f'Median_shot_accuracy_top1: {self.median_acc_top1:.3f} '
                  f'Low_shot_accuracy_top1: {self.low_acc_top1:.3f}', self.distil_log)

            self.many_acc_top1, \
            self.median_acc_top1, \
            self.low_acc_top1, \
            self.cls_accs = shot_acc(total_ba_preds,
                                     total_labels, 
                                     self.data['train'],
                                     acc_per_cls=True)
            ...
            print_w('Balanced --------', self.distil_log)
            print_w(f'Many_shot_accuracy_top1: {self.many_acc_top1:.3f} ' 
                 f'Median_shot_accuracy_top1: {self.median_acc_top1:.3f} '
                 f'Low_shot_accuracy_top1: {self.low_acc_top1:.3f}', self.distil_log)
            ...
            self.median_acc_top1, \
            self.low_acc_top1, \
            self.cls_accs = shot_acc(total_imba_preds,
                                     total_labels, 
                                     self.data['train'],
                                     acc_per_cls=True)
            ...
            print_w('Imbalanced ---------', self.distil_log)
            print_w(f'Many_shot_accuracy_top1: {self.many_acc_top1:.3f} ' 
                 f'Median_shot_accuracy_top1: {self.median_acc_top1:.3f} '
                 f'Low_shot_accuracy_top1: {self.low_acc_top1:.3f}', self.distil_log)
    ...
    def xERM_eval(self, phase='test'):
        ...
        for key, val in networks_defs.items():
            ...
            exp_dir = os.path.join(self.training_opt['exp_dir'], 'best_xERM_model_checkpoint.pth')
            print('Loading xERM model from %s' % (exp_dir))
            checkpoint = torch.load(exp_dir)          
        ...
        total_imba_logits = torch.empty(0).cuda()
        total_ba_logits = torch.empty(0).cuda()
        ...
        torch.cuda.empty_cache()
        with torch.no_grad():
            for step, (inputs, labels, indexes) in tqdm(enumerate(self.data[phase])):
                inputs, labels = inputs.cuda(), labels.cuda()
                # forward  model
                features = self.networks['feat_model'](inputs)
                logits_imba, _ = self.networks['classifier'](features, None, self.embed_mean)
                ...
                ba_right = tde_preds == labels
                imba_right = te_preds == labels
                preds_bound[ba_right] = tde_preds[ba_right]
                preds_bound[imba_right] = te_preds[imba_right]
                ...
                total_imba_logits = torch.cat((total_imba_logits, logits_imba), 0)
                total_ba_logits = torch.cat((total_ba_logits, logits_ba), 0)
                total_xERM_logits = torch.cat((total_xERM_logits, logits_xERM), 0)

        print(f'Imbalanced Acc {imba_accs.avg:.3f} '
              f'Balanced Acc {ba_accs.avg:.3f} '
        ...
        self.median_acc_top1, \
        self.low_acc_top1, \
        self.cls_accs = shot_acc(total_upper_bound,
                                 total_labels, 
                                 self.data['train'],
                                 acc_per_cls=True)
        ...
        print(f'Many_shot_accuracy_top1: {self.many_acc_top1:.3f} ' 
             ...
             f'Low_shot_accuracy_top1: {self.low_acc_top1:.3f}')
        # save logits
        # np.save('total_te_labels.npy', total_imba_preds.cpu().numpy())
        # np.save('total_tde_labels.npy', total_ba_preds.cpu().numpy())
        # np.save('total_stu_labels.npy', total_preds.cpu().numpy())
        # np.save('total_labels.npy', total_labels.cpu().numpy())
        # np.save('total_imba_logits.npy', total_imba_logits.cpu().numpy())
        # np.save('total_ba_logits.npy', total_ba_logits.cpu().numpy())
        # np.save('total_xERM_logits.npy', total_xERM_logits.cpu().numpy())

        # -----------------------------------------------------------------------------
        print('----------------------------------------------------------------')
        many, median, few = shot_precision(total_ba_preds,
                                           total_labels, 
                                           self.data['train'],
                                           acc_per_cls=False)
        print(f'Balanced Precision: Many {many:.4f} Median {median:.4f} Few {few:.4f}')

        many, median, few = shot_precision(total_imba_preds,
                                           total_labels, 
                                           self.data['train'],
                                           acc_per_cls=False)
        print(f'Imbalanced Precision: Many {many:.4f} Median {median:.4f} Few {few:.4f}')

        many, median, few = shot_precision(total_preds,
                                           total_labels, 
                                           self.data['train'],
                                           acc_per_cls=False)
        print(f'xERM Precision: Many {many:.4f} Median {median:.4f} Few {few:.4f}')
    ...
    def train(self):
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            print_write([self.training_opt['exp_dir']], self.log_file)
            # print learning rate
    ...
    def store_logits(self, phase):
        ...
        np.save(os.path.join(self.training_opt['exp_dir'],
    ...
    def eval(self, phase='val', save_feat=False):
        ...
        if phase == 'test':
            with open(os.path.join(self.training_opt['exp_dir'], 'cls_accs.pkl'), 'wb') as f:
    ...
    def load_model(self, exp_dir=None):
        exp_dir = self.training_opt['exp_dir'] if exp_dir is None else exp_dir
        ...
        if 'CIFAR' in self.training_opt['dataset']:
            ...
            if not exp_dir.endswith('.pth'):
                exp_dir = os.path.join(exp_dir, 'latest_model_checkpoint.pth')
        else:
            print('Validation on the best model.')
            if not exp_dir.endswith('.pth'):
                exp_dir = os.path.join(exp_dir, 'final_model_checkpoint.pth')

        print('Loading model from %s' % (exp_dir))
        checkpoint = torch.load(exp_dir)
        if 'latest' in exp_dir:
            model_state = checkpoint['state_dict']
        else:
            model_state = checkpoint['state_dict_best']
    ...
    def output_logits(self):
        filename = os.path.join(self.training_opt['exp_dir'], 'logits')
    ...
    def saving_feature_with_label_export(self):
        ...
        eval_features_dir = os.path.join(self.training_opt['exp_dir'],
